...
class GameBoard(object):
    ...
    def __init__(self, width=7, init_height=6, game_board=None):
        ...
        if game_board is None:
            self.the_board = [self.row_gen(width)
                              for row in range(init_height)]
            ...
            self.players_heuristics = copy.deepcopy(
                game_board.players_heuristics)
    ...
    def get_best_heuristic(self, symbol):
        ...
        columns = range(self.width)  # [0,1,2,3,4,5,6]
        heur_p1, heur_p2 = 0, 0
        get = lambda c_symbol, row_n, col_n: get_heuristic(
            self.the_board, c_symbol, row_n, col_n, terminal_test=False)
        ...
        if symbol not in self.players_heuristics:
            return 0
...
def heuristic_horizontal(
    ...
    # Left up diagonal then right down diagonal.
    directions = [left_right_vector]
    return heuristic_generic(
        the_board,
        symbol,
        row_index,
        column_index,
...
def heuristic_diagonal(
    ...
    if symbol != the_board[row_index][column_index]:
        return 0
    # Do the up negative slope diagonal first then positive slope. left, right
    # upwards.
    left_diagonal, right_diagonal = ((-1, -1), (1, 1)), ((-1, 1), (1, -1))
    # Left up diagonal then right down diagonal.
    directions = [left_diagonal, right_diagonal]
    return heuristic_generic(
        the_board,
        symbol,
        row_index,
...
def heuristic_generic(
    ...
    for cnt, diagonal in enumerate(dir_vectors):
        ...
        for diagonal_vector in diagonal:
            ...
            # Don't change the order of lambda calls.  You may violate left to
            # right.
    ...
    for cnt, plane_vect in enumerate(end_state):
        ...
        for inner_cnt, end_bound in enumerate(plane_vect):
            ...
            ) and left_bound() and right_bound():
...
def computer_player(node, state, player):
    ...
    move_weight, move = expectiminimax(
        node, 0, state, player, -float('inf'), float('inf'))
    return move
    ...
    if max_player:  # Max Player
        ...
        for value, child_move, child in child_gen(parent, state):
            if value < 40:  # Terminal Check
                value, t_move = minimax(
                    child, depth + 1, flip_player[state], player, alpha, beta)
            best_value, best_move = this_max(
                best_value, value, child_move, best_move)
            alpha = best_value
            # ALPHA/BETA CHECK
...
def expectiminimax(parent, depth, state, player, alpha, beta):
    ...
    if max_player:  # Max Player
        ...
        for value, child_move, child in child_gen(parent, state):
            ...
            # ALPHA/BETA CHECK
            # IF I'm Max.
            if (best_move is not None and beta <= alpha) or stop_on_chance:
                break
        return best_value, best_move
    else:  # Min Player
        best_value = float('inf')
...
class ConnectFour(object):
    ...
    def _next_turn_event(self):
        ...
        the turn.
        """
        column_move = self.player_map[
            self.turn](
            self.root_state,
            self.turn,
            self.turn)  # Key out the function.
        # Insert checks the move's heuristic.
        self.insert(column_move, self.turn)
